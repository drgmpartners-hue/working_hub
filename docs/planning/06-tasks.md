...
> 생성일: 2026-03-05 | 최종 수정: 2026-05-04
...
flowchart TD
    ...
    subgraph P3 [Phase 3: 투자 분석]
        ...
        subgraph P3RN [Resources - 신규]
            P3R7[P3-R7: Customers]
            P3R8[P3-R8: Customer Portfolios]
            P3R9[P3-R9: Portfolio Holdings]
            P3R10[P3-R10: Stock Market Data]
        end
    end
...
- **엔드포인트**:
  - GET /api/customers/:customer_id/portfolios (포트폴리오 목록)
  - POST /api/customers/:customer_id/portfolios (포트폴리오 생성)
  - GET /api/customers/:customer_id/portfolios/:id (포트폴리오 상세)
  - PUT /api/customers/:customer_id/portfolios/:id (포트폴리오 수정)
  - DELETE /api/customers/:customer_id/portfolios/:id (포트폴리오 삭제)
...
- **스펙**: 고객당 N개 포트폴리오 CRUD (계좌 단위, account_type 자유 설정)
- **Worktree**: `worktree/phase-3-portfolio`
- **TDD**: RED → GREEN → REFACTOR
- **의존**: P3-R7-T1
...
- **엔드포인트**:
  - GET /api/portfolios/:portfolio_id/holdings (보유 종목 목록)
  - POST /api/portfolios/:portfolio_id/holdings (종목 추가)
  - PUT /api/portfolios/:portfolio_id/holdings/:id (종목 수정)
  - DELETE /api/portfolios/:portfolio_id/holdings/:id (종목 삭제)
...
- **엔드포인트**:
  - GET /api/market/stocks/:code (현재가 + 기본정보)
  - GET /api/market/stocks/:code/chart (가격 차트 + 이동평균 + 거래량)
  - GET /api/market/stocks/:code/dividend (배당 정보)
  - GET /api/market/stocks/:code/fundamentals (PER, EPS, PBR)
  - GET /api/market/stocks/:code/supply-demand (외국인/기관 수급)
...
- **스펙**:
  - KIS(한국투자증권) API primary 연동
  - 네이버 금융 API secondary fallback (KIS 실패 시 자동 전환)
  - stock_type(stock/etf) 분기 처리
  - ETF 전용 필드(etf_holdings, etf_top10_ratio) 별도 처리
- **Worktree**: `worktree/phase-3-portfolio`
- **TDD**: RED → GREEN → REFACTOR
- **헌법**: `constitutions/fastapi/api-design.md` 준수
- **병렬**: P3-R7-T1과 병렬 가능
...
> 화면 명세: `specs/screens/customer-portfolio.yaml`

#### [ ] P3-S3-T1: 고객 포트폴리오 관리 UI 구현
- **담당**: frontend-specialist
- **화면**: /investment/portfolios
- **레이아웃**: full-width, 고객 선택 → 포트폴리오 탭 → 상세 뷰
- **컴포넌트**:
  - `CustomerSelector` - 기존 고객 DB 검색/선택 드롭다운
  - `PortfolioTabs` - 고객의 포트폴리오 탭 목록 (N개), 추가/삭제
  - `PortfolioSummary` - 총 투자금액, 예수금, 평가금액, 수익률 (배당 포함/미포함 토글)
  - `DividendOverview` - 월별 예상 배당수익 바 차트 + 연간 합계/YoY
  - `PerformanceChart` - 종목별 기준가 라인 차트 + 수익률 오버레이 (배당 토글)
  - `HoldingsTable` - 종목 구성 테이블 (유형/수량/단가/현재가/평가금액/수익률/배당수익률)
...
- **시나리오**:
  | 시나리오 | 액션 | 기대 결과 |
  |---------|------|---------|
  | 고객 선택 | 드롭다운에서 고객 선택 | 포트폴리오 탭 + 요약 로드 |
  | 배당 토글 | 배당 포함/미포함 전환 | 수익률 수치 + 차트 즉시 변경 |
  | 배당 현황 | 포트폴리오 선택 | 월별 배당 바 차트 + 연간 합계 표시 |
  | 종목 클릭 | HoldingsTable 행 클릭 | P3-S4 슬라이드 패널 열림 |
  | 포트폴리오 추가 | 추가 버튼 클릭 | 새 탭 생성 및 선택 |
...
- **화면**: /investment/portfolios
- **검증 항목**:
  - [ ] Field Coverage: customers.[id, name, phone, email] 존재
  - [ ] Field Coverage: customer_portfolios.[id, customer_id, name, total_invested, cash_balance] 존재
  - [ ] Field Coverage: portfolio_holdings.[id, portfolio_id, stock_code, stock_name, stock_type, holding_type, quantity, avg_price, purchase_date] 존재
  - [ ] Field Coverage: stock_market_data.[current_price, change_rate, dividend_yield, dividend_cycle] 존재
  - [ ] Endpoint: GET /api/customers 응답 정상
  - [ ] Endpoint: GET /api/customers/:id/portfolios 응답 정상
  - [ ] Endpoint: GET /api/portfolios/:id/holdings 응답 정상
  - [ ] Endpoint: GET /api/market/stocks/:code 응답 정상
  - [ ] Endpoint: GET /api/market/stocks/:code/chart 응답 정상
  - [ ] Toggle: 배당 포함/미포함 수익률 토글 동작
  - [ ] Navigation: HoldingsTable 행 클릭 → P3-S4 패널 열림
  - [ ] Navigation: Add from Recommend → /investment/stock-recommend 이동
  - [ ] Auth: 로그인 필수
  - [ ] Shared: HeaderBar 렌더링
...
### P3-S4: 개별 종목 상세 분석 화면 (Overlay)

> 화면 명세: `specs/screens/stock-detail.yaml`

#### [ ] P3-S4-T1: 종목 상세 분석 UI 구현
- **담당**: frontend-specialist
- **화면**: overlay (슬라이드 패널) - /investment/portfolios, /investment/stock-recommend에서 진입
- **레이아웃**: slide-panel, stock_type(stock/etf) 분기 렌더링
- **공통 컴포넌트**:
  - `StockDetailHeader` - 종목명, 티커, 현재가, 등락률, 타입 뱃지, 닫기 버튼
  - `PriceChart` - 이동평균선(5/20/60/120일) + 거래량 서브차트 + 기간 선택
- **주식(stock) 전용**:
  - `ValuationSection` - PER, EPS, PBR (업종 평균 대비 색상)
  - `SupplyDemandChart` - 외국인/기관 순매수 바 차트 (기간 선택)
  - `CompanyInfoSection` - 업종, 시가총액, 사업 요약
  - `RevenueChart` - 매출·영업이익 트렌드 (최근 4분기)
  - `RelatedStocksList` - 동일 업종 상위 종목 (클릭 시 해당 종목으로 전환)
- **ETF 전용**:
  - `EtfDividendSection` - 배당주기, 수익률, 안정성, 성장성
  - `EtfHoldingsChart` - 편입 종목 Top 10 비중 파이/바 차트
  - `EtfHoldingsTable` - 편입 기업별 비중·수익률
...
- **담당**: test-specialist
- **화면**: StockDetailPanel (overlay)
- **시나리오**:
  | 시나리오 | 액션 | 기대 결과 |
  |---------|------|---------|
  | 주식 타입 전체 섹션 | stock 종목 클릭 | valuation/supply_demand/company/revenue/related 표시, ETF 섹션 미표시 |
  | ETF 타입 전체 섹션 | ETF 종목 클릭 | dividend/holdings_chart/holdings_table 표시, 주식 전용 섹션 미표시 |
  | KIS API fallback | KIS API 실패 시 | 네이버 금융 전환, data_source="naver" 표시 |
  | 관련 종목 전환 | RelatedStocksList 클릭 | 패널 내용 교체 (닫힘 없이) |
...
- **화면**: StockDetailPanel (overlay)
- **검증 항목**:
  - [ ] Field Coverage: stock_market_data.[code, name, stock_type, current_price, change_rate] 존재
  - [ ] Field Coverage: stock_market_data.[price_history, ma5, ma20, ma60, ma120, volume_history] 존재
  - [ ] Field Coverage: stock_market_data.[per, eps, pbr] (주식 전용) 존재
  - [ ] Field Coverage: stock_market_data.[dividend_yield, dividend_cycle, dividend_stability, dividend_growth] 존재
  - [ ] Field Coverage: stock_market_data.[foreign_net_buy, institutional_net_buy] 존재
  - [ ] Field Coverage: stock_market_data.[etf_holdings, etf_top10_ratio] (ETF 전용) 존재
  - [ ] Endpoint: GET /api/market/stocks/:code/chart 응답 정상
  - [ ] Endpoint: GET /api/market/stocks/:code/fundamentals 응답 정상
  - [ ] Endpoint: GET /api/market/stocks/:code/supply-demand 응답 정상
  - [ ] Endpoint: GET /api/market/stocks/:code/dividend 응답 정상
  - [ ] Fallback: KIS API 실패 시 네이버 금융 자동 전환
  - [ ] Branch: stock_type에 따라 올바른 섹션 렌더링
  - [ ] Action: "포트폴리오에 담기" → SCR-10 연동
