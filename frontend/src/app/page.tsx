/**
 * 공개 랜딩 페이지 — / (로그인 불필요)
 * PASONA 구조: Problem → Agitation → Solution → Programs → Narrowing(차별화/신뢰) → Action(CTA)
 */
'use client';
...
const PROGRAMS = [
  { ic: I.users, t: '고객 정보 관리', d: '프로필·계좌·고유번호' },
...
export default function LandingPage() {
  ...
  // 로그인 상태면 대시보드가 기본 페이지 → / 진입 시 이동
  useEffect(() => {
    if (token) router.replace('/dashboard');
  }, [token, router]);

  const startHref = token ? '/dashboard' : '/register';
  const startLabel = token ? '대시보드로 이동' : '무료로 시작하기';

  // 로그인 상태에서는 랜딩 깜빡임 없이 대시보드로 이동
  if (token) {
    return (
      <div className="wh" style={{ minHeight: '100vh', display: 'grid', placeItems: 'center' }}>
        <span className="notion-spinner" />
      </div>
    );
  }
    ...
    <div className="wh">
      ...
      <header className="lp-topbar">
        <div className="lp-topbar-inner">
          <Link href="/" className="wh-brand" aria-label="Working Hub">
            <div className="mark" />
            <div className="name">Working<span> Hub</span></div>
          </Link>
          <div style={{ display: 'flex', gap: 'var(--sp-2)', alignItems: 'center' }}>
            <button className="wh-btn wh-btn-ghost wh-btn-sm" type="button" onClick={() => router.push('/login')}>로그인</button>
            <button className="wh-btn wh-btn-primary wh-btn-sm" type="button" onClick={() => router.push(startHref)}>{startLabel}</button>
          </div>
        </div>
      </header>
      ...
      <div className="wh-page">
        ...
        <section className="hero" style={{ borderRadius: 0 }}>
          ...
          <div className="hero-inner">
            <div className="hero-copy">
              <span className="section-tag eyebrow">금융 어드바이저 통합 워크스페이스</span>
              <h1>흩어진 업무 도구,<br /><span className="grad">하나로 끝내다.</span></h1>
              <p>고객 관리·포트폴리오 분석·수당 정산·콘텐츠 제작까지 — 어드바이저의 모든 반복 업무를 한 곳에서. 도구를 오가는 시간을 없애고, 고객에게 집중하세요.</p>
              <div className="hero-cta">
                <button className="wh-btn wh-btn-primary" type="button" onClick={() => router.push(startHref)}>{startLabel} {I.arrow}</button>
                <a className="wh-btn wh-btn-ghost" href="#features">기능 둘러보기</a>
              </div>
        ...
        {/* ===== PROBLEM ===== */}
        <section className="lp-section lp-center" id="problem">
        ...
        {/* ===== AGITATION ===== */}
        <section className="lp-section tight lp-center" id="why">
        ...
        {/* ===== SOLUTION ===== */}
        <section className="lp-section lp-center" id="solution">
          <div className="block-head">
        ...
        {/* ===== NARROWING (차별화/신뢰) ===== */}
        <section className="lp-section tight lp-center" id="numbers">
          <div className="block-head">
        ...
        <section className="lp-section" id="cta">
          <div className="lp-cta">
            <h2>지금, 더 가벼운 업무를 시작하세요.</h2>
            <p>설치 없이 바로. 어드바이저의 하루가 달라집니다.</p>
            <div className="row">
              <button className="wh-btn wh-btn-primary" type="button" onClick={() => router.push(startHref)}>{startLabel} {I.arrow}</button>
              <button className="wh-btn wh-btn-ghost" type="button" onClick={() => router.push('/login')}>로그인</button>
            </div>
          </div>
        </section>
        ...
        <footer className="lp-footer">
          <span>© {new Date().getFullYear()} Working Hub · 금융 어드바이저 통합 워크스페이스</span>
