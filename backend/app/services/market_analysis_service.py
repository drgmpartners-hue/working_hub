...
def build_analysis_comment_prompt(
    ...
    return (
        ...
        "반드시 포함할 내용:\n"
        "1. <b>투자 현황 요약</b> — 전체 투자 현황을 서술형으로 요약 (2줄)\n"
        "2. <b>종목별 분석</b> — 수익 종목은 왜 올랐는지, 손실 종목은 왜 떨어졌는지를 "
        "최근 시장 뉴스와 연결해서 쉽게 설명 (종목별 1-2줄씩)\n"
        "3. <b>포트폴리오 진단</b> — 현재 포트폴리오의 강점과 주의할 점 (2줄)\n"
        "4. <b>시장 전망</b> — 앞으로의 시장 전망과 참고 사항 (2줄)\n"
        + (
            "5. <b>리밸런싱 근거</b> — 수정 포트폴리오에서 비중을 늘린 종목의 기회요인, "
            "비중을 줄이거나 전액매도한 종목의 리스크 요인을 시장 상황과 연결하여 설명하고 "
            "수정 포트폴리오의 당위성을 확보해주세요 (3-4줄)\n"
            if changes_summary else ""
        )
        + "\n"
        "★ 출력 형식: HTML로 작성하세요.\n"
        "- 소제목은 <b> 태그로 굵게\n"
        "- 핵심 수치/키워드는 <span style=\"color:#1E3A5F;font-weight:600\"> 태그로 강조\n"
        "- 문단 구분은 <br><br>로\n"
        "- <ul><li> 목록 사용 가능\n"
        "- 인사말, 맺음말, 자기소개 등 사족은 절대 넣지 마세요. 분석 내용만 작성하세요.\n\n"
        "작성 규칙:\n"
...
def build_change_comment_prompt(
    ...
    return (
        ...
        "반드시 포함할 내용:\n"
        "1. <b>현재 현황</b> — 포트폴리오 상태를 1-2줄로 간략히 요약\n"
        "2. <b>비중 축소/매도 분석</b> — 매도하거나 비중을 줄인 종목별로:\n"
        "   - 최근 시장에서 무슨 일이 있었는지\n"
        "   - 왜 지금 줄이는 것이 합리적인지\n"
        "   - 앞으로 이 분야의 전망\n"
        "3. <b>비중 확대/신규 편입 분석</b> — 비중을 늘리거나 새로 추가한 종목별로:\n"
        "   - 이 분야의 최근 흐름\n"
        "   - 왜 지금 투자 비중을 높이는 것이 좋은지\n"
        "   - 앞으로 어떤 성장이 기대되는지\n"
        "4. <b>기대 효과</b> — 이번 변경으로 포트폴리오가 어떻게 좋아지는지 (1-2줄)\n"
        "5. <b>유의 사항</b> — 일반적인 이야기가 아니라, 이번 변경에서 구체적으로 주의할 위험요인, "
        "추후 있을 법한 시장 변동에 대한 대응 방향 등 실질적으로 도움이 되는 내용 (2-3줄)\n\n"
        "★ 출력 형식: HTML로 작성하세요.\n"
        "- 소제목은 <b> 태그로 굵게\n"
        "- 핵심 수치/키워드는 <span style=\"color:#1E3A5F;font-weight:600\"> 태그로 강조\n"
        "- 문단 구분은 <br><br>로\n"
